function trajectory_animation(travelPoints,angularOffsets,trialLengths,targetIndices,nRun)
...
% Inputs
%       travelPoints
%                   : 2 x (number of points), x,y coordinates of positions to
%                   be visited
%       trialSpecification
%                   : trialSpecification(1,:) stores angular offsets
%                   : trialSpecification(2,:) stores number of segments
%       nRun
%                   : number of the corresponding run
%       targetIndex
%                   : 1 x nTrial, indices of targets 
%
% Outputs
%       none
%
...
% grid map size given as the width and height of the rectangle in meters
xGridMapSize            = 40;
yGridMapSize            = 40;
...
for n = 1:nTrial
    ...
    gridMap                 = buildgridmap(xCenter,yCenter,xGridMapSize,yGridMapSize,angularOffsets(n));
    
    % draw the grid map in the background
    triplot(gridMap,'Color',[.7 .7 .7])
    hold on
    
    % set figure limits
    xlim([-.7*xArenaAxis + xCenter, .7*xArenaAxis + xCenter])
    ylim([-.7*yArenaAxis + yCenter, .7*yArenaAxis + yCenter])
    ...
    if n == 1
        ...
        startingSegmentIndex  = sum(trialLengths(1:n-1)) + 1;
    end
    
    endingSegmentIndex  = sum(trialLengths(1:n));
    ...
    for p = startingSegmentIndex : endingSegmentIndex
        ...
        if p == targetIndices(n)
             scatter(travelPoints(3,p),travelPoints(4,p),100,[.3 .3 .3],'filled','d')
        end 
    ...
    title(['Run ' num2str(nRun) ', Trial ' num2str(n) ', offset ' num2str(angularOffsets(n)) ' degrees'])
